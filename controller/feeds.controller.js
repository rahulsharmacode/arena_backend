...
const { Arena, Bookmarks, Liked, Comment, View } = require("../schema/arena.schema");
  ...
  try {
    ...
    const arenaObjects = await Promise.all(findData.map(async (arenaDoc) => {
      ...
      arena.like = totalLiked;
            ...
            const isViewed = await View.findOne({ user: req["rootId"], post: arena._id });
      arena.viewed = isViewed ? true : false;

      const totalViews = await View.countDocuments({ parent: arena._id });
      arena.view = totalViews;
